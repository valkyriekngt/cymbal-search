...
MODEL_1 = os.environ.get('MODEL_1', 'text-bison')
MODEL_2 = os.environ.get('MODEL_2', 'text-bison-32k')
LOCATION = "global"
...
def listEngines():
    try:   
        engines = []
        if ENGINE_1 == '':
            raise ValueError("ENGINE_1 is not specified in Environment Variable!")
        else:
...
def import_converted_to_ES():
    try:
        ...
        metadata, filename = create_json_converted(formData) # Create JSON file
        app.logger.info('~ Metadata Created:', str(metadata))
        gcs_json_url = create_upload_json(BUCKET_FOR_UPLOAD, metadata, filename) # Upload JSON to bucket
...
def purgeDocs():
    try:
        ...
        parent_value = f"projects/{PROJECT_ID}/locations/{LOCATION}/collections/default_collection/dataStores/{datastore_id}/branches/0"
...
def send_feedback():
    try:
        ...
        subject = "[Cymbalsearch] Register Interest"
        new_line = '\n'
        contents = f"Name: {name}{new_line}Company: {companyName}{new_line}Email: {companyEmail}{new_line}Contact: {mobileNumber}"
        send_email(subject, contents)
...
def start_convo():
    ...
    try: 
        results = converse_conversation(parent_value + "/conversations/-", user_input)
        return jsonify(results)
    except Exception as e:
