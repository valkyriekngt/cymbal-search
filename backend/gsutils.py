...
PROCESSOR_VERSION_COMPREHENSIVE = os.environ.get('PROCESSOR_VERSION_COMPREHENSIVE')
...
def uploadpdf(PROJECT_ID, BUCKET_NAME, file, formData):
    ...
    print("~ Creating JSON...")
    metadata, errors = create_metadata_json(PROJECT_ID, formData, mimeType, gcsDocUrl, fileName, numPages)
...
def bulk_upload_json(engineId, metadataList):
    ...
    jsonl_data = "\n".join([json.dumps(entry, ensure_ascii=False) for entry in obj_list])
