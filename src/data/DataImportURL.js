...
const DataImportURL = ({engine, isRefresh, setIsRefresh}) => {
  ...
  const handleConvertHTML = async () => {
    try {
      ...
      if (!response.ok) {
        setFileError(
          "Failed to convert the URL to PDF."
        );
        setIsConverting(false);
        return;
      }
  ...
  return (
    <div className="my-1">
      ...
      <div className="filter-sort-grid p-4 mb-3 shadow">
        <div className="">
          ...
          <div className="d-flex justify-content-start align-items-center mt-3">
            <button 
              ...
              style={{ backgroundColor: isLoading || isConverting ? "grey" : "" }}
            >
              Pdf Generator
            </button>
            <button 
        ...
        {pdfPreviewUrl && (
          <div className="pdf-preview-container mt-3">
            {isPDF ? (
            <ReactPdfViewer fileUrl={`data:application/pdf;base64,${pdfPreviewUrl}`}/>
            ) : (
            <embed className="embedded-pdf" src={`data:text/html;base64,${pdfPreviewUrl}`} type="text/html" /> 
            )}
          </div>
        )}
      </div>
      ...
      {fileError && (
        <Toast type="error" message={fileError} />
      )}
    </div>
  );
};
