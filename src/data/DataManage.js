...
const DataManage = ({engine, isRefresh, setIsRefresh}) => {
  ...
  const onRowsSelectionHandler = (selectedIds) => {
    const selectedNames = selectedIds.map((id) => documents.find((row) => row.id === id).name);
  ...
  return (
    <div className="my-1">
      <div className="filter-sort-grid mb-3 p-4 shadow">
        ...
        <div className="d-flex justify-content-between align-items-center">
          <div className="d-flex align-items-center">
            {/* Number of Documents */}
            <div>
              <h5 className="product-title">Number of Documents:
                <span className="mx-2" style={{ color: "#08144d" }}>{!isPurging && (documents.length)}</span>
              </h5>
            </div>
          </div>
          {isPurging && (
        ...
        <div className="mt-2">
          <button 
            ...
            disabled={documents.length === 0 || documentChecklist.length === 0 || isPurging} 
            style={{ backgroundColor: documents.length === 0 || documentChecklist.length === 0 || isPurging ? "grey" : "" }}
          >
          ...
          <button 
            ...
            style={{ backgroundColor: documents.length === 0 || isPurging? "grey" : "" }}
          >
            Purge Datastore
          </button>
        </div>
      ...
      {purgeError &&
        <Toast type="error" message={purgeError} />
      }
    </div>
  );
};
