...
Answer: `;
  ...
  useEffect(() => {
    ...
    if (
      !Array.from(urlParams.values()).some((paramValue) => paramValue)
    ) {
      return; 
    }
    ...
    const summary_result_count = parseInt(urlParams.get("summary_result_count"));
    const max_snippet_count = parseInt(urlParams.get("max_snippet_count"));
    const max_extractive_answer_count = parseInt(urlParams.get("max_extractive_answer_count"));
    const max_extractive_segment_count = parseInt(urlParams.get("max_extractive_segment_count"));
  ...
  const handleFacetsChange = (facet, isChecked, category) => {
    ...
    if (isChecked) {
      setFacetChecklist((prevFacetChecklist) => {
        ...
        updatedFacetChecklist[category] = [...prevFacetChecklist[category], facet];
        return updatedFacetChecklist;
      });
    } else {
      setFacetChecklist((prevFacetChecklist) => {
        const updatedFacetChecklist = { ...prevFacetChecklist };
        updatedFacetChecklist[category] = prevFacetChecklist[category].filter((item) => item !== facet);
        return updatedFacetChecklist;
      });
    }
  ...
  */
  const handleSearch = async (query, searchState) => {
    try {
      ...
      const idList = searchState === 1 ? checkedItems.map((item) => item.id) : []; // use checkedItems only for Regenerate
      ...
      // Set Summary, Query
      setSummary(data[0].summary === "" ? "No Summary Generated." : data[0].summary);
      setOriginalQuery(query);
      setQuery(data[0].corrected_query !== "" ? data[0].corrected_query : query);
  ...
  const handleNextPage = async () => {
    if (isLoading || isLoadingRegenerate) return;
    if (currentPageIndex < fetchedPages.length) setSearchResults(fetchedPages[currentPageIndex]);
  ...
  return (
    <>
      ...
      <Container class1="store-wrapper home-wrapper-2 py-3">
        <div className="row" >
          ...
          <div className="col-9">
            <SearchHeader
              ...
              correctedQuery={searchResults.length > 0 ? searchResults[0].corrected_query : ""}
              isLoading={isLoading}
                  ...
                  <div className="container-xxl">
                    <div className="d-flex justify-content-center align-items-center">
                      <div className="d-flex align-items-center">
                        <GrPrevious className="fs-5 arrow-button"
                          onClick={handlePrevPage}
                          hidden={currentPageIndex === 1}
                          disabled={isLoadingRegenerate || isLoading}
                          style={{ cursor: !isLoadingRegenerate && !isLoading ? "pointer" : "" }}
                        />
                        {fetchedPages.map((pageData, index) => (
                          <span
                            key={index}
                            className={`shadow page-number ${currentPageIndex === index + 1 ? "active" : ""}`}
                            onClick={() => handlePageNumberClick(index + 1)}
                          >
                            {index + 1}
                          </span>
                        ))}
                        {(currentPageIndex < fetchedPages.length || nextPageToken) &&
                          <GrNext className="fs-5 arrow-button"
        ...
        {/* ToggleSidebar */}
        <div>
          <ToggleSidebar toggleIcon={toggleSidebar} isSidebarExpanded={isSidebarExpanded} />
        </div>
        ...
        {isChatbotOpen && (
          <Followup onClose={closeChatbot} engine={engine} initialQuery={originalQuery}></Followup>
        )}
      </Container>
    </>
  );
