...
const Result = ({ index, item, isChecked, numChecked, summaryCount, onCheckboxChange, indexOffset, currentPageIndex}) => {
  ...
  const summary_brief = (typeof item.filter_summary_brief === 'string' || item.filter_summary_brief instanceof String) ? item.filter_summary_brief : "";
  ...
  return (
    <>
      <div className="product-card position-relative p-4 shadow">
        <div className="product-details">
          <div className="product-title-container">
            <h5 className="product-title">[{resultIndex}] {title}</h5>
          ...
          <div className="product-link urilink" onClick={handlePopupOpen}>{filename}</div>

          {/* DocAI Summary */}
          {summary_brief &&
          <Snippet
            title={`DocAI Summary`}
            snippet={summary_brief.trim()}
          />
          }
        ...
        <div className="action-bar position-absolute">
          <div className="d-flex flex-column gap-15 m-1 py-3">
            ...
            <button className="border-0 bg-transparent" onClick={handleViewOpen}>
              <img src={view} alt="view" />
            </button>

            <button className="border-0 bg-transparent" onClick={handleSegmentOpen}>
      ...
      {showPopup && (
        <Popup onClose={handlePopupClose} title={title} description={item.filter_summary_comprehensive} uriLink={item.uri_link} />
      )}
      {showView && (
        <Popup onClose={handleViewClose} title={title} description="" uriLink={item.uri_link} />
      )}
      {showSegment && (
        <Popup onClose={handleSegmentClose} title={title} description={segment} />
      )}
    </>
  );
};
