...
const Popup = ({ onClose, title, description, uriLink }) => {
  ...
  const summary = (typeof description === 'string' || description instanceof String) ? description.trim() : "";
      ...
      {pdfUrl &&
        <div>
          {fileExtension === "pdf" || fileExtension === "application/pdf" ? (
            ...
            <br/>
            <embed className="embedded-pdf" src={`${pdfUrl}#view=fitH`} type={fileExtension} /> 
            </>
          )}
        </div>
      }
